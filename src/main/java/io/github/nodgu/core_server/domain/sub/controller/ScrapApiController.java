...
import io.github.nodgu.core_server.domain.sub.dto.ScrapListViewResponse;
...
public class ScrapApiController {
    ...
    public ResponseEntity<List<ScrapListViewResponse>> findAllMyScraps(@AuthenticationPrincipal User user) {
        List<ScrapListViewResponse> scraps = scrapService.findAllScrapsByUser(user.getId())
                .stream()
                .map(ScrapListViewResponse::new)
                .toList();
    ...
    @PostMapping("/myScrap")
    public ResponseEntity<Scrap> addScrap(@RequestBody AddScrapRequest request, @AuthenticationPrincipal User user) {
